...
class WikidataClassEnum(enum.Enum):
    ...
    MULTINATIONAL_MILITARY_COALITION = "wd:Q100906234"  # https://www.wikidata.org/wiki/Q100906234
    ...
    SOURCES_OF_INTERNATIONAL_LAW = "wd:Q2635077"  # https://www.wikidata.org/wiki/Q2635077
    SUPRANATIONAL_UNION = "wd:Q1335818"  # https://www.wikidata.org/wiki/Q1335818
    WARFARE_AND_ARMED_CONFLICTS = "wd:Q71266556 wd:Q198 wd:Q467011"  # armed conflict, war, invasion

    @staticmethod
    def _qcodes_for(selected_types: list[str] | None) -> str:
    ...
    def to_sparql_values_block(selected_types: list[str] | None) -> str:
        qcodes = WikidataClassEnum._qcodes_for(selected_types)
        return f"VALUES ?itemType {{ {qcodes} }}"

    @staticmethod
    def root_qcodes_for(selected_types: list[str] | None) -> list[str]:
        """Return raw Q-code strings (no 'wd:' prefix) for the given API types."""
        raw = WikidataClassEnum._qcodes_for(selected_types)
        return [token.replace("wd:", "") for token in raw.split()]
